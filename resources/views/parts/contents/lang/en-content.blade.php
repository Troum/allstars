...
<div class="content-wrapper inner-wrapper">
    <div class="left-wrapper">
        <div class="column-left">
            <div class="column-inner-wrapper">
                <div class="picture">
                    <img src="{{asset('images/'.$event->cover)}}" alt="{{$event->en_title}}">
                </div>
                @foreach($event->additional as $item)
                    <div class="schedule">
                        <a class="inner-info" href="">
                            <p class="date"><strong>{{$item->date->format('F')}} {{$item->date->format('j')}} </strong></p>
                            <p class="time">{{$item->time->format('H:i')}}</p>
                        </a>
                        <a class="inner-info" href="">
                            <p class="city">{{$item->en_city}}</p>
                            <p class="stage">&nbsp;/&nbsp;</p>
                            <p class="stage">{{$item->en_place}}</p>
                        </a>
                    </div>
                    <div class="tickets">
    ...
    <div class="column-right">
        ...
        <div class="button-wrapper">
            <a href="https://www.youtube.com/results?search_query={{$event->en_title}}" class="red-button more-video">
            ...
            <div class="cards">
                @foreach($events as $value)
                    @if($value->id != $event->id && $value->hidden_status == 1)
                        <a class="card wow fadeInUp" href="/en/events/{{$value->slug}}">
