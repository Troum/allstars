<footer id="footer">
    <div class="content-wrapper">
        <div class="footer-top">
            <ul class="footer-left">
                <li class="logo-wrapper">
                    <a href="/">
                    </a>
                </li>
                <li>
                    @if(app()->isLocale('ru'))
                    <a href="/about">
                        @lang('content.about')
                    </a>
                    @else
                    <a href="/en/about">
                        @lang('content.about')
                    </a>
                    @endif
                </li>
                <li>
                    <a href="/public/documents/smi.docx">
                        @lang('content.for_media')
                    </a>
                </li>
            </ul>
            <ul class="footer-right">
                <li >
                    <h3 style="color: #fff">@lang('content.contacts')</h3>
                </li>
                <li>
                    <a href="tel:+375296501133">+375 (29) 650 11 33</a>
                         <br>
                    <a href="tel:+375297631111">+375 (29) 763 11 11</a>
                </li>
                <li>
                    <a href="mailto:info@allstars.by">
                        info@allstars.by
                    </a>
                </li>
            </ul>
        </div>
        <div class="footer-center">
            <div class="social-wrapper">
                <a class="animated fadeInRight" href="https://www.facebook.com/allstars.by" target="_blank" style="animation-delay: 0.5s;">
                    <img src="data:image/svg+xml;base64," alt=""/>
                </a>
                <a class="animated fadeInRight" href="https://www.instagram.com/allstarsby/" target="_blank" style="animation-delay: 0.6s;">
                    <img src="data:image/svg+xml;base64," alt=""/>
                </a>
                <a class="animated fadeInRight" href="https://vk.com/allstars_by" target="_blank" style="animation-delay: 0.8s;">
                    <img src="data:image/svg+xml;base64," alt=""/>
                </a>
            </div>
            <div class="input-e-mail">
                <form action="/subscribe" method="POST">
                    {{csrf_field()}}
                    <input id="email-subscribe" name="email-subscribe" type="email" placeholder="@lang('content.email')" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$" required>
                    <button type="submit">@lang('content.subscribe')</button>
                </form>
            </div>
            <div  style="margin:0px auto !important">
                <div id="vk_allow_messages_from_community" style="margin:0px auto !important; text-align:center"></div>
                <script type="text/javascript">
                    VK.Widgets.AllowMessagesFromCommunity("vk_allow_messages_from_community", {}, 2158488);
                </script>
            </div>
        </div>
    </div>
    <div class="footer-bottom">
        <p>© {{date('Y')}} AllStars.by</p>
    </div>
</footer>